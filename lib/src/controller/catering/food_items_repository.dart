    ...
    try {
      ...
      final response = await FoodItemsProvider().addFoodItem(
        ...
        data: FoodItemRequest(
            ...
            foodItemUnit: selectedUnitId!.id!,
...
//--------------load food item types-------------
  ...
  List<FoodItemUnitResponse> foodItemUnits = [];
  List<DropDownValue> foodItemUnitsList = [];
  Future<void> foodItemUnitList({required String token}) async {
    try {
      isloading = true;
      final response =
          await FoodItemsProvider().loadFoodItemUnits(token: token);
      if (response != null) {
        foodItemUnits = response;
        foodItemUnitsList = response
            .map((e) => DropDownValue(id: e.id, value: e.name))
            .toList();
        notifyListeners();
      }
  ...
  DropDownValue? selectedUnitId;
  void changeUnitId(value) {
    selectedUnitId = value;
  }
  //=-=-=-=-=-=-= Init State Loading =-=-=-=-==-=-=-=

  bool isEdit = false;
  void initStateLoading({FoodItemListResponse? data, int? index}) {
    if (data == null) {
      ...
      selectedId = DropDownValue(
          id: foodItemTypesList[index!].id, value: foodItemTypes[index].name); 
      selectedUnitId = DropDownValue(
          id: foodItemUnitsList[index].id, value: foodItemUnits[index].name);
